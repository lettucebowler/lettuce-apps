      ...
      {#each query.data.pages ?? [] as page (page)}
        {#each page.results as gameResult (gameResult)}
          <div class="flex w-full flex-[1_1_200px] flex-col gap-2 rounded-2xl">
            <h2
              class="text-snow-300 flex justify-between text-center text-xl font-medium"
            >
              <span class="text-left">#{gameResult.gameNum}</span><span
                class="text-right"
    ...
    <nav class="mx-4 flex justify-end gap-2">
      ...
      {#if data.next}
        <a
          ...
          title="Next"
          class="text-snow-300 ml-auto text-lg font-medium">Next</a
        >
      {/if}
    </nav>
  {/if}
